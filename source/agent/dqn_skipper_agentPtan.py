...
from source.helpers.log import log_eval_skipper_agent
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--wrapper', default='Skipper', type=str, help="Leave as default. Used for sorting results.")
    parser.add_argument('--seed', type=int, default=1)
    parser.add_argument('--win_reward', type=float)
    parser.add_argument('--max_repeat', default=3, type=int)
...
def main(args):
    with wandb.init(project=args.project_name, entity='cbellinger', mode=args.wandb_mode, config=args) as run:
        ...
        results, skipper_agent, env = train(wandb.config)
        after_training(wandb.config, results, skipper_agent, env, "lastTrained_")
        load_policy(skipper_agent.behave_model, skipper_agent.skip_model, run.dir, detailed_name="best_")
        after_training(wandb.config, results, skipper_agent, env, "best_")

def after_training(args, result, skipper_agent, env,detailed_name="lastTrained_"):
    log_eval_skipper_agent(args, env, skipper_agent, detailed_name)

def save_fn(policy_behave, policy_skip, run_dir, detailed_name="best_"):
    torch.save(policy_behave.state_dict(), os.path.join(run_dir, detailed_name+'dqn_behave_policy.torch'))
    torch.save(policy_skip.state_dict(), os.path.join(run_dir, detailed_name+'dqn_skip_policy.torch'))

def load_policy(policy_behave, policy_skip, run_dir, detailed_name="best_"):
...
def train(args):
    ...
    if args.env_name == "PongNoFrameskip-v4":
        env = make_env(args.env_name, args.obs_cost, args.max_repeat, False, args.full_ext_reward, max_episode_steps=None)
        env = atari.make_skipper_env(env)
        obs_size = env.observation_space.shape
    else:
        env = make_env(args.env_name, args.obs_cost, args.max_repeat, False, args.full_ext_reward, max_episode_steps=args.max_episode_steps)
    ...
    if args.noisyNet == 1:
        ...
        net_skip = dqn_model.NoisyMLP_DQN(obs_size+1,n_actions_skipper,args.hidden_size).to(args.device)
        tgt_net_skip = ptan.agent.TargetNet(net_skip)
    ...
    optimizer_skip = optim.Adam(net_skip.parameters(), args.lr)
    ...
    if args.resume_path:
        load_policy(agent.behave_model, agent.skip_model, args.resume_path, detailed_name="")
    ...
    while frame_idx < args.total_timesteps:
        ...
        for reward, ext_rewards, int_rewards, steps, last_rewards, fullObs_rewards, measures in exp_source.pop_verbose_rewards_steps():
            ...
            if best_ep_reward is None or best_ep_reward < reward:
                save_fn(net_behave, net_skip, args.run_dir, "best_")
        ...
        if args.prioReplay == 1:
            ...
            loss_v_behave, sample_prios_bh_v,loss_v_skip, sample_prios_sk_v = common.calc_loss_prio_skipper_dqn(batch, batch_weights, net_behave, tgt_net_behave, net_skip, tgt_net_skip, args.gamma**args.n_step, device=args.device,  doubleQ=args.doubleQ)
        else:
            batch = buffer.sample(args.batch_size)
            loss_v_behave, loss_v_skip= common.calc_loss_skipper_dqn(batch, net_behave, tgt_net_behave, net_skip, tgt_net_skip, args.gamma**args.n_step, device=args.device, doubleQ=args.doubleQ)
        
        loss_v_behave.backward()
        optimizer_behave.step()
        loss_v_skip.backward()
        optimizer_skip.step()
        if args.prioReplay == 1:
            #TODO currently ignoring the behave and skipper loss. How to handle the combo of behave and skipper?
            buffer.update_priorities(batch_indices, (sample_prios_bh_v+sample_prios_sk_v))
        #
    ...
    save_fn(net_behave, net_skip, args.run_dir,detailed_name="lastTrained_")
